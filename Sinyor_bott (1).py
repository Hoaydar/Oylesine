...
# ÖZEL GRUP ID'sİ (kendi grubunun ID'sini buraya yaz)
SPECIAL_GROUP_ID = -4851033886  

# Token ayarlama
async def set_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Sadece özel grupta çalışsın
    if update.effective_chat.id != SPECIAL_GROUP_ID:
        await update.message.reply_text("❌ Bu komut sadece özel grupta kullanılabilir.")
        return

    if not context.args:
        await update.message.reply_text("⚠️ Kullanım: /settoken YENI_TOKEN")
        return
    ...
    with open("token.json", "w", encoding="utf-8") as f:
        json.dump({"token": new_token}, f, ensure_ascii=False, indent=2)

    await update.message.reply_text(f"✅ Token başarıyla güncellendi:\n\n<code>{new_token}</code>", parse_mode="HTML")

...
# ================== Telegram ==================
TOKEN = "8275693889:AAGxq5vm1-mKIAXiHI6Q-r6O3xUEbi53AAk"  # kendi tokenini koy
...
# ❌ ESKİ GLOBAL VERİTABANI BAĞLANTISI KALDIRILDI. ARTIK BAĞLANTI İHTİYAÇ DUYULDUĞUNDA AÇILIP KAPATILACAK.

# ---- Bonus Alan Kullanıcılar (telegram kontrol)----
BONUS_USERS_FILE = "bonus_users.json"
print("🚀 Kod başladı")

# Veritabanı bağlantı parametrelerini bir fonksiyona taşıyalım
def get_db_connection():
    """Yeni bir MariaDB bağlantısı oluşturur."""
...
def has_taken_bonus(user_id: int) -> bool:
    ...
    try:
        with open(BONUS_USERS_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        # JSON dosyasında int olarak kayıtlıysa kontrol et
        return user_id in data or str(user_id) in data
    except FileNotFoundError:
        return False
    except json.JSONDecodeError:
        # Boş veya bozuk dosya durumunda
        return False
...
async def betco_find_user(username: str):
    ...
    if data1 and data1.get("HasError"):
        return {"ok": False, "error": data1.get("AlertMessage")}
...
📢 Kanalımıza katılmak için lütfen aşağıdaki butona tıklayınız """
    ...
    # Yanıt olarak fotoğraf göndermek yerine, botun ilk mesajı olarak göndermek daha uygun olabilir.
    if update.message and update.message.chat.type == "private":
        try:
            await update.message.reply_photo(photo=photo_url, caption=caption_text, reply_markup=InlineKeyboardMarkup(keyboard))
             ...
             await update.message.reply_text(caption_text, reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        await update.message.reply_text("Lütfen bu işlemi DM üzerinden yapın.")
...
async def give_freebet(client_id: int):
    # Bu fonksiyon artık genel give_bonus tarafından ele alındığı için teknik olarak gereksiz,
    # ancak mevcut yapıyı korumak için bırakılabilir.
    return await give_bonus(client_id, "freebet")
...
async def handle_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # API Hata kontrolü
    if api_result and not api_result.get("ok") and api_result.get("error"):
        if "401 Unauthorized" in api_result.get("error"):
             await update.message.reply_text("❌ Betco token geçersiz. Lütfen `/settoken <yeni_token>` komutu ile güncelleyiniz.")
        else:
            await update.message.reply_text(f"❌ Betco sorgusunda bir hata oluştu: {api_result.get('error')}")
        return
    ...
    try:
        # 🟢 YENİ: Bağlantıyı sorgu anında kuruyoruz
        ...
        # 🔴 ÖNEMLİ: İşlem bitince cursor'ı ve bağlantıyı kapat
        cursor.close()
        conn.close()

    except mariadb.Error as e:
        # Bağlantı açma veya sorgu hatası
        if conn:
            try: conn.close()
            except: pass
        await update.message.reply_text(f"❌ Veritabanı bağlantı/sorgu hatası: {e}")
        return
    except Exception as e:
        # Genel hata
        if conn:
        ...
        await update.message.reply_text(f"❌ Veritabanı sorgusunda beklenmedik bir hata: {e}")
        return
    ...
    # --- 3) DB eşleşmesi varsa (API'den alınan bilgileri tekrar kontrol et) --- 
    if rows:
        await update.message.reply_text("✅ TC doğrulandı, diğer filtrelere geçiliyor...")

        # API'de kullanıcı var mı kontrolü (Tekrar yapılmasına gerek yok, başta yapıldı ama garantileyelim)
        ...
        # Tekrar kullanıcı detaylarını çek
        detail = {}
        try:
            detail_resp = await betco_get_user_by_id(client_id)
        ...
        if created_date_str:
            try:
                ...
                if created_date < cutoff:
                    await update.message.reply_text(
                        f"❌ {cutoff.strftime('%d.%m.%Y')} tarihinden önce kayıt olduğunuz için bonus hakkınız bulunmamaktadır."
                    )
                    return
            except Exception as e:
                print(f"CreatedLocalDate parse hatası: {e}, value={created_date_str}")
                await update.message.reply_text("❌ Kayıt tarihi formatı okunamadı.")
                return
        ...
        def has_active_noncancelled_bonus(bonus_items):
            ...
            if isinstance(bonus_items, dict):
                ...
                items = bonus_items
            for b in items:
                # Burada sadece iptal edilmemiş (CancellationNote yok) herhangi bir bonusu kontrol ediyoruz.
                if b and b.get("CancellationNote") is None:
                    # Alternatif bir kontrol, örneğin StatusId 10:Accepted, 11:Active
                    # if b.get("StatusId") in [10, 11]: return True
                    # Veya basitçe, bir kez bonus almış mı?
                    return True
            return False
    ...
    # Bu blok, DB kontrolünden (if not rows) sonra API'den kullanıcı bulunduysa çalışır.
...
async def bonus_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("bonus_"):
        try:
            ...
            await query.edit_message_text("❌ Geçersiz bonus verisi.")
            return

        # ✅ İlk defa alıyorsa API’ye istek atıyoruz
        await query.edit_message_text(f"⏳ {bonus_type.upper()} yükleniyor, lütfen bekleyin...")
        resp = await give_bonus(client_id, bonus_type)

        if resp.get("HasError"):
            # Önceki mesajı düzenleyerek hata mesajını göster
            await query.edit_message_text(f"❌ **{bonus_type.upper()} yüklenemedi!**\n\nHata: {resp.get('AlertMessage')}")
        else:
            # Başarı mesajı
            if bonus_type == "freespin":
                msg = "✅ **500 FreeSpin** hesabınıza başarıyla yüklendi!"
            elif bonus_type == "freebet":
                msg = "✅ **50 FreeBet** hesabınıza başarıyla yüklendi!"
            else:
                msg = "✅ Bonus hesabınıza başarıyla yüklendi!"
...
async def broadcast_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kullanıcıları int listesi olarak normalize et
    target_users = []
    for uid in users:
        try:
    ...
    await update.message.reply_text(f"📢 **{len(target_users)}** kullanıcıya resimli duyuru gönderiliyor...")
    ...
    for uid in target_users:
        try:
            ...
            # Hata kodu 403 (Forbidden) genellikle kullanıcının botu engellediği anlamına gelir
            if "Forbidden: bot was blocked by the user" in str(e):
                 print(f"❌ {uid} kullanıcısına mesaj gönderilemedi: Kullanıcı botu engelledi.")
            else:
                print(f"❌ {uid} kullanıcısına resimli mesaj gönderilemedi: {e}")
            failed_count += 1
        # Flood sınırlarını aşmamak için küçük bir bekleme
        await asyncio.sleep(0.05) 

    await update.message.reply_text(
        f"✅ Resimli duyuru gönderimi tamamlandı.\n"
...
if __init__ == "__main__":
    ...
    app.job_queue.run_once(
        lambda ctx: asyncio.create_task(token_reminder_task(app)), 
        0
    )

...
# ================== Telegram ==================
TOKEN = "8350444240:AAHbhxHeI2AbPZTXCTe90qkadp7ZzE1sdtI"  # kendi tokenini koy
...
def turkish_strtoupper(text: str) -> str:
    turkish_map = {
    ...
    for char in text:
        if char in turkish_map:
            result += turkish_map[char]
        else:
            result += char.upper()
    return result

# ================== TC Doğrulama ==================
async def verify_with_api(tc: str, isim: str, soyisim: str, dogumyili: int):
    url = "http://5.181.1.27:8000/verify"
    headers = {
        "Content-Type": "application/json",
        "X-API-KEY": "4A2312355SVBFEWSASTUYZXVBNSAZX"
    }
    payload = {
        "dogumyili": dogumyili,
        
        "isim": isim.upper(),   # Büyük harf
        "soyisim": soyisim.upper(), # Büyük harf
        "tcno": tc,
    }
    print(f"[VERIFY API] Gönderilen: {payload}")
        ...
        try:
            async with session.post(url, headers=headers, json=payload) as resp:
                ...
                try:
                    ...
                    # 🔹 dogrulama_sonucu varsa True/False döndür
                    return result.get("dogrulama_sonucu", False)
                except Exception:
                    print(f"[VERIFY PARSE ERROR] {text[:200]}")
                    return False
        except Exception as e:
...
async def handle_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        print("🔹 Betco API find_user sonucu:", json.dumps(api_result, ensure_ascii=False, indent=2))
        ...
        if client_id:
            # ---- Betco get_user_by_id ----
            detail_resp = await betco_get_user_by_id(client_id)
            print("🔹 Betco API get_user_by_id sonucu:", json.dumps(detail_resp, ensure_ascii=False, indent=2))
        ...
        if not DocNumber or not BirthDate:
            await update.message.reply_text("❌ Kullanıcının TC veya doğum tarihi bilgisi bulunamadı.")
            return
        ...
        full_name = FirstName + (f" {MiddleName}" if MiddleName else "")

        # ---- TC Doğrulama ----
        verify_result = await verify_with_api(
            tc=DocNumber,
            isim=turkish_strtoupper(full_name.strip()),
            soyisim=turkish_strtoupper(LastName.strip()),
            dogumyili=birth_year
        )
        print("🔹 TC Doğrulama API Yanıtı:", verify_result)

        # 🔹 Sadece dogrulama sonucu True ise ilerle
        if verify_result is True:
            await update.message.reply_text("✅ TC doğrulandı, bonus seçeneklerine geçiliyor...")
        else:
            await update.message.reply_text(
                "❌ TC doğrulama başarısız! Bu nedenle bonus alamazsınız.\n\n"
                "Lütfen bilgilerinizin doğru olduğundan emin olun veya destek ile iletişime geçin."
            )
            return 
